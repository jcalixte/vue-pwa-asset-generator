...
const outputFolder = (options.output || '').trim('/') || '.'

const resize = (name, width, height = undefined, showSize = true) => {
  if (!height) {
    height = width
  }

  const size = showSize ? `-${width}x${height}` : ''
  ...
  sharp(options.asset)
    ...
    .toFile(`./${outputFolder}/${name}${size}.png`, function (err) {
      if (err) {
        console.error(err)
      }
    });
